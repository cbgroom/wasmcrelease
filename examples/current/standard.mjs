...
const installedArg=process.argv.indexOf('--installed');
const packageBase=installedArg<0?new URL('../../',import.meta.url):pathToFileURL(resolve(process.argv[installedArg+1])+'/');
const root=new URL('standard/wasmc-std/1.4.0/',packageBase).href;
const provider=await load(new URL('standard/corelib/4.8.0/corelib.wasm',packageBase).href);
const lib=await load(root+'artifact.wasm');
const plan=await load(root+'typed-resource-plan.json');
preflightCoreArtifact('standard-provider',provider);
preflightCoreArtifact('standard',lib);
