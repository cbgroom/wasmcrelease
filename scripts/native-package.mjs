...
export async function verify(directory, source, target) {
  ...
  if (!/^[a-f0-9]{40}$/.test(source) || manifest.schema !== 'wasmc-native-package/v2' ||
      ...
      manifest.run_backend !== 'wasmi' || manifest.native_build_backend !== 'wasmtime-aot' ||
...
if (mode === 'build') {
  ...
  await writeFile(join(directory, 'manifest.json'), JSON.stringify({
    schema: 'wasmc-native-package/v2',
    source,
    target,
    profile: profileName,
    compiler_sha256: compilerDigest,
    compiler_authority: 'portable-core-wasm',
    run_backend: 'wasmi',
    native_build_backend: 'wasmtime-aot',
    stable: false,
    toolchain: command('rustc', ['+1.96.0', '-Vv']),
    files,
  }, null, 2) + '\n');
}
if (mode === 'build' || mode === 'verify') {
  ...
  if (!receipt.accepted || receipt.byte_parity_cases !== 5 || receipt.wasmi_run_cases !== 4 ||
      receipt.native_execution_cases !== 4 || receipt.no_clobber_cases !== 9) throw new Error('invalid behavior receipt');
  console.log(JSON.stringify({ accepted: true, source, target, profile: profileName, ...receipt }));
}
