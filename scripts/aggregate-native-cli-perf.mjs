...
if (reports.some(r => r.commit !== commit)) throw new Error('mixed performance commits');
const median = values => {
...
const current = {
  ...
  policy: 'GitHub-hosted timings are comparative observations. Source/Wasm identity and behavior are hard gates; timing regression is advisory until repeated history establishes a stable threshold.',
  summary,
  reports,
};
let history = [];
...
const table = [
  ...
  '|---|---:|---:|---:|---:|---:|---:|',
  ...summary.map(row => `| ${row.platform} | ${(row.cli_bytes / 1048576).toFixed(2)} MiB | ${row.build_wasm_geomean_ms.toFixed(3)} ms | ${row.native_miss_geomean_ms.toFixed(3)} ms | ${row.native_hit_geomean_ms.toFixed(3)} ms | ${row.run_wasmi_p50_ms?.toFixed(3) ?? '-'} ms | ${row.native_run_p50_ms?.toFixed(3) ?? '-'} ms |`),
].join('\n');
const markdown = `# WAsmC public performance\n\nCommit: \`${commit}\`  \nMeasured: ${current.measured_at}  \nPlatforms: ${reports.length}  \nCanonical corpus: ${current.corpus_count}\n\n${table}\n\nTimings on GitHub-hosted runners are comparative observations, not absolute SLA claims. Corpus identity, generated Wasm identity, and behavior are hard gates.\n`;
const htmlRows = summary.map(row => `<tr><td>${row.platform}</td><td>${(row.cli_bytes / 1048576).toFixed(2)} MiB</td><td>${row.build_wasm_geomean_ms.toFixed(3)} ms</td><td>${row.native_miss_geomean_ms.toFixed(3)} ms</td><td>${row.native_hit_geomean_ms.toFixed(3)} ms</td><td>${row.run_wasmi_p50_ms?.toFixed(3) ?? '-'} ms</td><td>${row.native_run_p50_ms?.toFixed(3) ?? '-'} ms</td></tr>`).join('');
const html = `<!doctype html><meta charset="utf-8"><title>WAsmC performance</title><style>body{font-family:system-ui,sans-serif;max-width:1200px;margin:40px auto;padding:0 20px}table{border-collapse:collapse;width:100%}th,td{border:1px solid #ddd;padding:8px;text-align:right}th:first-child,td:first-child{text-align:left}code{background:#f4f4f4;padding:2px 4px}</style><h1>WAsmC public performance</h1><p>Commit <code>${commit}</code> · ${current.measured_at}</p><p>GitHub-hosted timings are comparative observations; artifact identity and behavior are hard gates.</p><table><thead><tr><th>Platform</th><th>CLI</th><th>build Wasm</th><th>native miss</th><th>native hit</th><th>run/Wasmi</th><th>native run</th></tr></thead><tbody>${htmlRows}</tbody></table><p><a href="latest.json">latest.json</a> · <a href="history.json">history.json</a> · <a href="latest.md">Markdown</a></p>`;
...
console.log(JSON.stringify({ accepted: true, commit, platforms: reports.length, runMedian, nativeMedian, buildMedian }));
