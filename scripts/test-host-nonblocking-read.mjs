...
const expected = readiness ? 31 : 25;
if (!result || Number(result[1]) !== expected) throw new Error('completion test coverage changed; review receipt contract');
const paths = ['lib.rs', 'owner_supervisor.rs', 'nonblocking_tcp.rs', ...(readiness ? ['readiness.rs'] : [])]
...
const receipt = { schema_version: 1, source, source_dirty: dirty, platform: platform(), arch: arch(),
  ...
  scope: 'preopened exclusive TCP read owner + scoped supervisor; optional OS owner wait, no guest Future/executor, shared reactor or mobile proof' };
