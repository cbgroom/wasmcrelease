...
const expected = readiness ? 38 : 25;
if (!result || Number(result[1]) !== expected) throw new Error('completion test coverage changed; review receipt contract');
const paths = ['lib.rs', 'owner_supervisor.rs', 'nonblocking_tcp.rs', ...(readiness ? ['readiness.rs', 'read_reactor.rs'] : [])]
...
const receipt = { schema_version: 1, source, source_dirty: dirty, platform: platform(), arch: arch(),
  ...
  shared_reactor_controls: readiness ? 7 : 0,
  profile: readiness ? 'native-readiness' : 'default',
  scope: 'preopened exclusive TCP read owner + scoped supervisor; optional bounded OS reactor, no guest Future/executor or mobile proof' };
