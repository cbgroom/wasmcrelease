...
fn portable_std_wasmi() -> Result<(), Box<dyn Error>> {
    ...
    let root =
        std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("../../../admission/portable-std-v0");
    ...
    for file in ["rust.wasm", "wasmc.wasm"] {
        ...
        assert_eq!(
            p.get_typed_func::<(i32, i32), i32>(&s, "provider_domain_init")?
                .call(&mut s, (127, 7))?,
            0
        );
        ...
        let mut l = Linker::new(&e);
        for x in lib.exports(&s) {
            l.define("wasmc:lib/wasmc.std@1.4.1", x.name(), x.into_extern())?;
        }
...
fn portable_std_wasmtime() -> Result<(), Box<dyn Error>> {
    ...
    for file in ["rust.wasm", "wasmc.wasm"] {
        ...
        assert_eq!(
            p.get_typed_func::<(i32, i32), i32>(&mut s, "provider_domain_init")?
                .call(&mut s, (127, 7))?,
            0
        );
        ...
        let exports = p
            .exports(&mut s)
            .map(|x| (x.name().to_owned(), x.into_extern()))
            .collect::<Vec<_>>();
        ...
        let exports = lib
            ...
            .collect::<Vec<_>>();
        for (name, value) in exports {
            linker.define(&mut s, "wasmc:lib/wasmc.std@1.4.1", &name, value)?;
        }
        ...
        let mut out = f
            .ty(&s)
            .results()
            .map(|ty| Val::default_for_ty(&ty).unwrap())
            .collect::<Vec<_>>();
        let mut calls = 0;
        for _ in 0..128 {
            for index in [0u32, 1, 2, 99, u32::MAX] {
