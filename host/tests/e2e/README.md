...
Run `cargo build --release --locked --manifest-path host/tests/e2e/rust/Cargo.toml`
then `node host/tests/e2e/test.mjs host/tests/e2e/rust/target/release/wasmc-lib-host-e2e`
...
[session/completion guard](../../runtime/completion/README.md) in both JS and Native;
guard windows and terminal records are released before computation.
The reviewed driver now issues its own scoped binding identity with JS WebCrypto
or Native OS randomness, failing closed on entropy errors. No identity is passed
from the parent harness into the Native chain;
[issuance obligations](../../runtime/completion/SCOPED_IDENTITY.md) remain
