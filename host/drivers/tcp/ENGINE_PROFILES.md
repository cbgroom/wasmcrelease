...
```sh
cargo build --release --locked --manifest-path host/tests/e2e/rust/Cargo.toml --features wasmtime-engine
...
node host/drivers/tcp/server-test.mjs host/tests/e2e/rust/target/release/tcp-server-reference host/tests/e2e/rust/target/release/resident-app-reference --wasmtime
```
